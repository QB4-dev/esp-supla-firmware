...
static void exp_input_poll(void *arg)
{
	...
	if(data->prev_level && !level){
	    event = (data->active_level == ACTIVE_LOW) ?
	            EXP_INPUT_EVENT_INIT : EXP_INPUT_EVENT_DONE;
	}
	/* rising edge */
	if(!data->prev_level && level){
        event = (data->active_level == ACTIVE_HIGH) ?
                EXP_INPUT_EVENT_INIT : EXP_INPUT_EVENT_DONE;
    }
    ...
    if(level == data->active_level && !data->hold && (tick - data->init_tick) > pdMS_TO_TICKS(data->hold_time) ){
